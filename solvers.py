        ...
        for split in range(splits):
            ...
            if filter_corr == True:
                with np.errstate(divide='ignore'):
                    try: 
                        corr_matrix = np.mean(np.corrcoef(self.quad_obj), axis = 0)
                        corr_matrix[np.isnan(corr_matrix)] = 1 
                        print("objective contains corr filter")
                        objective = cp.Minimize(0.5 * cp.quad_form(w, cp.psd_wrap(P)) + q.T @ w + w @ corr_matrix) 
                    except np.exceptions.AxisError as e:
                        corr_matrix = 0
                    #print("Expection raised, no correlation setting the matrix to 0")
                    #print("Corr Matrix is", 0,"\n", "P shape is", P.shape, "\n" , "W shape is", w.shape, "\n")
                    #print("No Corr Matrix")
                        objective = cp.Minimize(0.5 * cp.quad_form(w, cp.psd_wrap(P)) + q.T @ w + w * corr_matrix)
            else: 
                 objective = cp.Minimize(0.5 * cp.quad_form(w, cp.psd_wrap(P)) + q.T @ w)
